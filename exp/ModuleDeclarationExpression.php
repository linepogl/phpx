...
class ModuleDeclarationExpression extends Expression {
	public $Access;
	...
	public function Compile(Compiler $compiler) {
		...
		echo '<li>Compiling module declaration ' . $qname->ToString() . ': ';

		$x = $compiler->Assembly->FindQName($qname);
		if (!is_null($x)){
			$compiler->Validator[] = new CompileTimeException('Cannot redeclare '.$qname->ToString().' as module.',$this->GetSource());
		...
		if (is_null($parent))
			$compiler->Assembly->Modules[$x->Name] = $x;
		else
			$parent->Modules[$x->Name] = $x;
